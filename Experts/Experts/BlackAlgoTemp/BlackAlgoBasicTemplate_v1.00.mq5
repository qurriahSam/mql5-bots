...
//---
   ...
   if(_Digits == 5 || _Digits == 3 || _Digits == 1)
      ...
      P = 1; // To account for 5 digit brokers
...
// -------------------- Collect most current data --------------------
   ...
   numberOfLongSmaData = CopyBuffer(longSmaControlPanel, 0, 0, 3, longSmaData); // Collect most current SMA(10) Data and store it in the datatable/array longSmaData[]
   ...
   if(PositionSelect(_Symbol) == false) // We have no open position
     {
      ...
      if(shortSma2 < longSma2 && shortSma1 >= longSma1) // Rule to enter long trades
      ...
      if(shortSma2 > longSma2 && shortSma1 <= longSma1) // Rule to enter short trades
